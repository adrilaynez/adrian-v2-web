...
const enFile = 'src/i18n/en.ts';
const esFile = 'src/i18n/es.ts';

const en = fs.readFileSync(enFile, 'utf8');
const es = fs.readFileSync(esFile, 'utf8');

const newBigramNarrative = `    bigramNarrative: {
        hero: {
...
function replaceBlock(content) {
    // Determine the line ending of the file (CRLF or LF)
    ...
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes('bigramNarrative: {')) {
            startIdx = i;
            break;
        }
    }
    ...
    if (startIdx === -1) {
        console.error("Could not find bigramNarrative block");
        process.exit(1);
    }

    // Walk down to find matching closing brace
    let braceCount = 0;
    ...
    if (endIdx === -1) {
        console.error("Could not find closing brace for bigramNarrative block");
        process.exit(1);
    }

    // Format the new string with proper line endings and trailing comma if needed
    // Just make sure the final string fits exactly.
    let replacement = newBigramNarrative.split('\\n').join(nl) + ',';

    lines.splice(startIdx, endIdx - startIdx + 1, replacement);
    return lines.join(nl);
}

fs.writeFileSync(enFile, replaceBlock(en));
fs.writeFileSync(esFile, replaceBlock(es));
