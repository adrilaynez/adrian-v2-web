...
function replaceExactRange(file, startLine, endLine, replacement) {
    ...
    // Splice parameters: start, deleteCount, items to add
    // startLine is 1-indexed, so -1 to get the 0-idx.
    lines.splice(startLine - 1, endLine - startLine + 1, ...replacementLines);

    fs.writeFileSync(file, lines.join(nl));
}

// en.ts and es.ts both have the old `bigramNarrative: {` from line 996 to `    },` at line 1061.
replaceExactRange('src/i18n/en.ts', 996, 1061, newBigramNarrative);
replaceExactRange('src/i18n/es.ts', 996, 1061, newBigramNarrative);

console.log("Successfully replaced lines 996-1061 in en.ts and es.ts.");
